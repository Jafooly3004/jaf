...
╰━━┻╯╱╰┻╯
"
zenity --info --title "Just Another Framework [JAF] - Version 0.5.1" --text "JAF - DISCLAIMER: This script is designed to act as a TRIAGE TOOL ONLY it is not a replacement for analysis. \n \nEscape sequence is Ctrl+C \n \nPlease press OK to accept and continue." --width=350 --height=150
sleep 0.2
TOTALSTART="$(date +%s)"
...
then
  ...
  zenity --info --title "INSTALLATION DETECTION" --test "Standalone installation detected and used." --width=350 --height=150
fi
...
zenity --list --radiolist --title "COMMAND SELECTION" --text "Please select a COMMAND option, when finished, select QUIT" --height=650 --width=250 --cancel-label "ABORT" --column "Select" --column "Menu Item" FALSE PWHASH FALSE HP FALSE MALFIND FALSE SUSPROC FALSE SOCKSCAN FALSE HARDWARE FALSE HPDLLDUMP FALSE CON FALSE MANUAL FALSE QUIT > $temp2
  selection=$(cat $temp2)
  case $selection in
    PWHASH)
      START="$(date +%s)"
