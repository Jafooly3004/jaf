...
echo "VERSION: 1.0"
echo "AUTHOR: Chris HAWKINS - Staffordshire University - 2022"
echo "
╱╱╭┳━━━┳━━━╮
╱╱┃┃╭━╮┃╭━━╯
╱╱┃┃┃╱┃┃╰━━╮
...
#VOLATILITY INSTALLATION TESTING
echo "Testing that Volatility2.6 installed correctly. Please stand by."
./vol -h &> /dev/null
if [[ $? == 0 ]];
then
  volatility=$"./vol"
  echo "SUCCESSFUL: Installation confirmed."
  zenity --info --title "INSTALLATION DETECTION" --text "SUCCESS: Installation confirmed." --width=350 --height=150
else
    volatility=$(zenity --entry --title "INSTALLATION DETECTION" --text "Installation Failed. \nREQUIRED: Please enter the full pathway of your volatility script:")
    $volatility -f test.vmem pslist &> /dev/null
      if [[ $? == 0 ]];
      then
