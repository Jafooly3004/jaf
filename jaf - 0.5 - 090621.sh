...
╰━━┻╯╱╰┻╯
"
zenity --info --title "Just Another Framework [JAF] - Version 0.5" --text "JAF - DISCLAIMER: This script is designed to act as a TRIAGE TOOL ONLY it is not a replacement for analysis. \n \nEscape sequence is Ctrl+C \n \nPlease press OK to accept and continue." --width 350 --height 150
sleep 0.2
TOTALSTART="$(date +%s)"
...
volatility () {
  /home/user/volatility
}
#VOL.PY INSTALLED VERSION
#volatility () {
#  vol.py
#}
#USER INPUTS
...
#TRACKING: fi @ 87
    ...
    1)
      ...
      PROFILE=$(volatility -f $FILE imageinfo | grep Profile | awk '{print $4}' | sed 's/,*$//g') && echo "Profile location successful."
...
#TRACKING: fi @ ?
            then
              ...
              echo "ERROR: Profile could not be located."
                ...
                case $? in
#TRACKING: 0 @ 103 , 1 @  164 , -1 @ 167 , esac @ ?
...
#TRACKING: fi @ 163
                                        then
                                          ...
                                          case $? in
#TRACKING: 0 @ 128, 1 @ 156 , -1 @ 159 , esac @ 162
                                            0)
