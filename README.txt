Just Another Framework - JAF - V1.0 - README FILE
...
REQUIREMENTS:
BASH Shell
Volatility Framework (Volatiltity2 - Python installation, you can check if this is properly installed by using the vol.py command from within the CLI)
Zenity tool - can be installed using your distro's package manager e.g. apt install zenity

INSTALLATION:
Git Clone the JAF files to the / directory of your machine - this should create a directory in the location that you called git from named: jaf
...
USAGE:
JAF can be run by simply calling the jaf.sh script from the CLI, remember to make it executable in your normal way!
...
CON - Runs the CONSOLES volatility command to attempt to extract command history from the given image.

NAMEDPROCINFO - This command will ask the user to name a process, and will then output specific information about that process for analysis. Examples of information provided are PID and open/closure times.

IPFINDER - This command will obtain and print all local IP addresses (IPv4 and IPv6) for the machine at the time of imaging and will print these, along with port and socket information.
This will NOT print all connections nor state, it is for IP Location only.

NAME - This command will analyse the registry of the imaged machine and use that information to automatically locate and extract the name of the machine that was imaged. This command varies depending on the Windows OS that is in use and therefore will ask the user what version of Windows was imaged.

METERTRACE - This command will check the common connection traces associated with MSF's metertrace and will provide information on any connection that matches these traits. It will also automatically dump the process and DLLs for that  process so that these can be sent to a virus checker to ascertain if they are compromised or malicious.
